...
import com.example.allu.hrm_superviser.Adapter.Recy_Adapter_SupVis_Req;
...
public class ViewYOurRequests extends AppCompatActivity {
    String TAG = ViewYOurRequests.class.getSimpleName();
    Utils utils;
    RequestQueue queue;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_view_your_requests);
        ...
        RequestsList = (RecyclerView)findViewById(R.id.recy_yourrequests);
        RequestsList.setHasFixedSize(false);
        RequestsList.setItemAnimator(new DefaultItemAnimator());
        RequestsList.setLayoutManager(new GridLayoutManager(this,1));

        requirementsArrayList = new ArrayList<>();
        Requirment_clickInterface requirment_clickInterface = new Requirment_clickInterface() {
            @Override
            public void onClick(SupVis_Requirements requirements) {
                Log.e(TAG,"clicked");
            }
        };
        adapterSupVisReq = new Recy_Adapter_SupVis_Req(this,requirementsArrayList,requirment_clickInterface);
    ...
    void LoadYourRequests(){
        utils.ShowDialog();
        JSONObject params = new JSONObject();
        try {
            params.put(DataAttributes.REQUEST_OPTION, "get_request");
            params.put(RespAttr.COL_su_suid,utils.GetUserDetails().SID);
        } catch (JSONException e) {
            e.printStackTrace();
        }
        Log.e(TAG,params.toString()+" "+URL.URL_Supervisor);
        JsonObjectRequest request = new JsonObjectRequest(Request.Method.POST, URL.URL_Supervisor, params, new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject jsonObject) {
                try {
                    if (jsonObject.getInt(DataAttributes.RESPONSE_CODE) == DataAttributes.RESPONSE_SUCCESS){
                        ...
                        utils.CloseDialog();
                        RequestsList.setAdapter(adapterSupVisReq);
                    }else{
                        utils.CloseDialog();
                        utils.Toast(jsonObject.getString(DataAttributes.RESPONSE_MESSAGE));
                    }
