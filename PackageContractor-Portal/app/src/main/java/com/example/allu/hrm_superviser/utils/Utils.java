...
import com.example.allu.hrm_superviser.Pojo.Aadhar;
import com.example.allu.hrm_superviser.Pojo.Su_user;
import com.example.allu.hrm_superviser.Pojo.User;
import com.example.allu.hrm_superviser.UI.Activities.LoginActivity;
...
public class Utils {
    ...
    public void Login(Su_user user){
    ...
    public Su_user GetUserDetails(){
        if(LoginStatus()){
            String json = preferences.getString(DataAttributes.PREFERENCE_USER_OBJECT,"");
            Su_user user = gson.fromJson(json,Su_user.class);
            return user;
        }else{
            return null;
        }
